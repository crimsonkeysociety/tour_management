...
class TourForm(forms.Form):
    ...
    counts_for_requirements = forms.BooleanField(initial=True)
    ...
    def clean_guide(self):
        ...
        if not models.Person.objects.filter(**(utilities.current_kwargs(semester=semester, year=time.year))).filter(id=guide.id):
            if time > now:
                ...
                verb = 'was not'
            raise exceptions.ValidationError((u'This member {0} active at the time of this tour.'.format(verb)), code='invalid')
        else:
            return guide
...
class ShiftForm(forms.Form):
    ...
    late = forms.BooleanField(required=False)
    length = forms.IntegerField(max_value=999, required=False) # Tour length, in minutes
    counts_for_requirements = forms.BooleanField(initial=True, required=False)
    ...
    def clean_person(self):
        ...
        if not models.Person.objects.filter(**(utilities.current_kwargs(semester=semester, year=time.year))).filter(id=person.id):
            if time > now:
                ...
                verb = 'was not'
            raise exceptions.ValidationError((u'This member {0} active at the time of this shift.'.format(verb)), code='invalid')
        else:
            return person

    def clean(self):
        if self.cleaned_data.get('counts_for_requirements') and self.cleaned_data.get('source') in settings.NON_REQUIREMENT_SOURCES:
            raise exceptions.ValidationError('A shift from the source you selected cannot count for requirements.')
    ...
    def clean_guide(self):
        ...
        if not models.Person.objects.filter(**(utilities.current_kwargs(semester=semester, year=time.year))).filter(id=guide.id):
            if time > now:
                ...
                verb = 'was not'
            raise exceptions.ValidationError((u'This member {0} be active at the time of this tour.'.format(verb)), code='invalid')
        else:
            return guide
...
class PersonBulkForm(PersonForm):
    class Meta(PersonForm.Meta):
        fields = ('first_name', 'last_name', 'email', 'harvard_email', 'phone', 'year', 'member_since', 'house',)
