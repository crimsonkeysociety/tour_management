...
def profile(request, year=None, semester=None):
	...
	# TOURS:
	tours = person.tours.filter(**current_semester_kwargs).order_by('time')
	completed_and_upcoming_tours_num = tours.filter(missed=False).count()
	person.past_tours = tours.filter(time__lte=now).order_by('time')
	person.upcoming_tours = tours.filter(time__gt=now).order_by('time')
	...
	# SHIFTS:
	shifts = person.shifts.filter(**current_semester_kwargs).order_by('time')
	completed_and_upcoming_shifts_num = shifts.filter(missed=False).count()
	person.past_shifts = shifts.filter(time__lte=now).order_by('time')
	person.upcoming_shifts = shifts.filter(time__gt=now).order_by('time')
